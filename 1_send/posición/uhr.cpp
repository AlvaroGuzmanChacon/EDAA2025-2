 ...
 *
 * Things to set up:
 * 0. Includes: include all files to be tested, CHECK
 * 1. Time unit: in elapsed_time,
 * 2. What to write on time_data, CHECK
 * 3. Data type and distribution of RNG, CHECK
 * 4. Additive or multiplicative stepping, CHECK
...
int main(int argc, char *argv[])
{
    ...
    time_data << "n,t_mean,t_stdev,t_Q0,t_Q1,t_Q2,t_Q3,t_Q4,experiment" << std::endl;

  // Test configuration goes here
  std::vector<int> vec(upper+1);
  std::iota(vec.begin(), vec.end(), 1);
    ...
    for (n = lower; n <= upper; n += step) {
        ...
        time_data << n << "," << mean_time << "," << time_stdev << ",";
        time_data << q[0] << "," << q[1] << "," << q[2] << "," << q[3] << "," << q[4] << ",";
        time_data << "exponential" << std::endl;
    }
