...
int main(int argc, char *argv[])
{
  ...
  std::mt19937 rng(rd());
  std::uniform_int_distribution<int> u_distr; // change depending on app

  // File to write time data
  std::ofstream time_data;
  time_data.open(argv[1]);
  time_data << "n,t_mean,t_stdev,t_Q0,t_Q1,t_Q2,t_Q3,t_Q4,Heap,Análisis" << std::endl;
  ...
  // Analisis Peor Caso (insertar elemento menor a todos: -1).
  for (n = lower; n <= upper; n *= step) {
    mean_time = 0;
    time_stdev = 0;

    // Test configuration goes here
    std::vector<int> heap_vector(vec.begin(), vec.begin()+n);
    FibonacciHeap<int> Heap(heap_vector);
    //BinaryHeap Heap(vector_rd);
    //BinomialHeap Heap(vector_rd);
    //FibonacciHeap<int> Heap(vector_rd);
    ...
    time_data << "Fibonacci" << "," << "Peor Caso" << std::endl;
  }
  ...
  for (n = lower; n <= upper; n *= step) {
    ...
    time_data << n << "," << mean_time << "," << time_stdev << ",";
    time_data << q[0] << "," << q[1] << "," << q[2] << "," << q[3] << "," << q[4] << ",";
    time_data << "Fibonacci" << "," << "Amortizado" << std::endl;
  }
  /*
    for (n = lower; n <= upper; n += step) {
        mean_time = 0;
        ...
        // Test configuration goes here
        std::vector<int> heap_vector(vector_rd.begin(), vector_rd.begin()+n);
        //BinaryHeap Heap(vector_rd);
        BinomialHeap Heap(vector_rd);
        //FibonacciHeap<int> Heap(vector_rd);
